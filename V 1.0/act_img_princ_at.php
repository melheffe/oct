...
if (empty($errores)) {
			$query = "UPDATE at SET
			pic_at = '$desti' WHERE id_at=$id_at";
			
			mysql_query($query);
		
			header("Location: act_img_princ_at.php?id_at=$id_at");
			exit();	
      ...
      <table width="0" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          <td width="461"><form action="<?=$_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
            <p>
              <input name="id_at" type="hidden" id="x" value="<?=$id_at?>" />
            </p>
